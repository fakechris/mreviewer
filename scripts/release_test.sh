...
require_pattern ".github/workflows/release.yml" "RELEASE_PR_TOKEN" "release workflow must support a dedicated token for formula sync pushes and PRs"
...
require_pattern ".github/workflows/release.yml" "env\\.RELEASE_PR_TOKEN == ''" "release workflow should only workflow-dispatch CI when a dedicated token is unavailable"
require_pattern ".github/workflows/release.yml" 'gh workflow run CI --ref "\$branch"' "release workflow must retain the CI dispatch fallback when no dedicated token is configured"
