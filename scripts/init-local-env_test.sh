...
grep -q '^LLM_PROVIDER=minimax$' "$output_file"
grep -q '^LLM_API_KEY=minimax-token-for-test$' "$output_file"
grep -q '^LLM_BASE_URL=https://api.minimaxi.com/anthropic$' "$output_file"
grep -q '^LLM_MODEL=MiniMax-M2.7-highspeed$' "$output_file"
